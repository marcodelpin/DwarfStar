...
.PHONY: all help clean test test-metal-session-batch test-mxfp4-cuda test-cuda-session-batch test-cuda-mixed-batch dspark-acceptance dspark-verify-depth mtp-verify-depth cpu cuda cuda-spark cuda-generic cuda-regression test-cuda-prefill-mixed-score-bound test-cuda-router-general strix-halo rocm
...
cuda-regression: tests/cuda_long_context_smoke tests/test_cuda_ordered_f16_dispatch tests/test_cuda_prefill_mixed_score_bound tests/test_cuda_router_general
	./tests/cuda_long_context_smoke
	./tests/test_cuda_ordered_f16_dispatch
	./tests/test_cuda_prefill_mixed_score_bound
	./tests/test_cuda_router_general
...
# Router selection at expert counts other than 256 (issue #427). The 256/6/1.5
# cases are the control: they validate the host reference against the untouched
# fixed-size kernels before it is trusted on the general one.
tests/test_cuda_router_general.o: tests/test_cuda_router_general.c ds4_gpu.h
	$(CC) $(CFLAGS) -I. -c -o $@ tests/test_cuda_router_general.c

tests/test_cuda_router_general: tests/test_cuda_router_general.o ds4_cuda.o $(MMQ_OBJS)
	$(NVCC) $(NVCCFLAGS) -o $@ $^ $(CUDA_LDLIBS)

test-cuda-router-general: tests/test_cuda_router_general
	./tests/test_cuda_router_general
